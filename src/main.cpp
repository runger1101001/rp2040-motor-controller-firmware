...
#define DEFAULT_VOLTAGE_LIMIT 5.0f
...
void setup() {
  ...
  motor0.LPF_velocity.Tf = motor1.LPF_velocity.Tf = 0.01f;
  motor0.foc_modulation = motor1.foc_modulation = FOCModulationType::SinePWM;
  motor0.controller = motor1.controller = MotionControlType::velocity_openloop;
...
void loop() {
  ...
  if (ts + 1000uL < millis()) {
    ...
    SimpleFOCDebug::print(" ang0: ");
    SimpleFOCDebug::println(sensor0.getAngle());
    count = 0;
  }
